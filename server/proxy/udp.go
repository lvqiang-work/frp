...
func (pxy *UDPProxy) Run() (remoteAddr string, err error) {
	...
	url := fmt.Sprintf("%s?clientToken=%s&proxyName=%s&remotePort=%d", pxy.serverCfg.PortCheckUrl, pxy.ClientToken, pxy.cfg.ProxyName, pxy.cfg.RemotePort)
	xl.Info("port_check_url:[%s]", url)
	...
	if string(body) != "yes" {
		err = fmt.Errorf("[" + pxy.cfg.ProxyName + "][" + strconv.Itoa(pxy.cfg.RemotePort) + "]udp proxy check error")
		return
	} else {
