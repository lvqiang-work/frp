...
func (svr *Service) RegisterControl(ctlConn net.Conn, loginMsg *msg.Login) (err error) {
	...
	//token auth key
	url := fmt.Sprintf("%s?clientToken=%s&userName=%s", svr.cfg.TokenAuthUrl, loginMsg.ClientToken, loginMsg.User)
	xl.Info("token auth url:[%s]", url)
	resp, err := http.Get(url)
	if err != nil {
		xl.Warn("token auth server error: %v", err)
		if strings.Contains(err.Error(), "connection refused") {
			err = fmt.Errorf("token auth server refused")
			return
		} else {
			err = fmt.Errorf("token auth server failed")
			return
		}
	}
